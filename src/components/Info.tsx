...
export default function Info() {
  ...
  return (
    <div className="grid grid-cols-1 items-start gap-8 py-12 md:grid-cols-2 md:gap-12">
      {/* Texto */}
      <div>
        <h2 className="mb-4 text-[1.3125rem] font-semibold lg:text-[1.75rem]">
          {description.heading}
        </h2>
        ...
        <AnimatePresence initial={false}>
          {open && (
            ...
            >
              {rest.map((p, i) => (
                <p key={i} className="mt-4 text-[0.9375rem] leading-relaxed text-ink/90">
                  {p}
                </p>
              ))}
            </motion.div>
        ...
        >
          {open ? <Minus className="size-4" strokeWidth={3} /> : <Plus className="size-4" strokeWidth={3} />}
