...
export default function PurchasePanel() {
  const [selected, setSelected] = useState(
    purchase.versions.find((v) => v.selected)?.id ?? purchase.versions[0].id
  );
  ...
  return (
    <div className="flex flex-col">
      ...
      {/* Título */}
      <h1 className="mb-5 text-[1.75rem] font-semibold leading-tight lg:text-[2.375rem]">
        {title}
      </h1>
      ...
      {/* Link upgrade */}
      <a href="#" className="mt-3 text-sm font-semibold text-primary underline hover:no-underline">
      ...
      <div className="mt-5 flex items-center gap-2 text-sm">
        ...
        <span className="rounded bg-mist px-2 py-0.5 text-ink/80">{purchase.edition}</span>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        >
          <Heart className="size-6" strokeWidth={2.5} fill={wished ? "currentColor" : "none"} />
        </motion.button>
      </div>
